...
public class DemoTiltBall94587Setup extends Activity
{
	...
	// Define the maximum number of laps you want to display and generate laps
	final static int MAX_NUM_LAPS = 1024;
	final static String[] NUM_LAPS = generateNumLapsArray(MAX_NUM_LAPS);
	...
	Spinner spinOrderOfControl, spinGain, spinPathMode, spinPathWidth, spinNumLaps; //added code for numlaps
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		Log.i(MYDEBUG, "Got here! (DemoTiltBall94587Setup - onCreate)");
		...
		// added code to initialize and populate numlaps spinner
		spinNumLaps = (Spinner) findViewById(R.id.paramNumLaps);
		ArrayAdapter<CharSequence> adapterNumLaps = new ArrayAdapter<CharSequence>(this, R.layout.spinnerstyle, NUM_LAPS);
		spinNumLaps.setAdapter(adapterNumLaps);
		spinNumLaps.setSelection(0);
	}
	...
	public void clickOK(View view) 
	{
		...
		Intent i = new Intent(getApplicationContext(), DemoTiltBall94587Activity.class);
		i.putExtras(b);
		startActivity(i);
	...
	// Method to generate more laps
	private static String[] generateNumLapsArray(int maxNumLaps) {
		String[] numLapsArray = new String[maxNumLaps + 1]; // Add 1 to include 0 as an option
		for (int i = 0; i <= maxNumLaps; i++) {
			numLapsArray[i] = String.valueOf(i); // Convert the integer to a string
		}
		return numLapsArray;
	}
